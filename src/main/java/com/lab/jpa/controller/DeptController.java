...
public class DeptController {
    ...
    @GetMapping(value = {"/", //查詢全部使用
        "/{id}", //根據id查詢單筆使用(給修改連結用)
...
//        dept.setName("Hello");
    ...
    @PostMapping(value = "/")
    public String create(@ModelAttribute("dept") Department dept, BindingResult result,
            Model model) {
        //  數據驗證 @ModelAttribute("dept") 一定要加
        validation.validate(dept, result);  //  result存放驗證後的結果
        if (result.hasErrors()) {
